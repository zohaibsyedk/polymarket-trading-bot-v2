...
def evaluate_entry(cfg: BotConfig, quote: QuoteSnapshot, elapsed_in_market_sec: int, cash_available: float) -> EntryDecision:
    ...
    if elapsed_in_market_sec < entry_start or elapsed_in_market_sec >= cfg.market_interval_seconds:
        return EntryDecision(False, reason=f"outside_final_{cfg.final_entry_window_seconds}s_window")
    ...
    if top < cfg.entry_min_price_threshold:
        return EntryDecision(False, reason="waiting_for_0.75_threshold")
    ...
    return EntryDecision(True, side=side, price=px, size_usd=size, reason="final_50s_threshold_then_higher_side")
